<?php
	...
	$sql = " SELECT id,catid,url FROM ".$config['db_prefix']."_news_crawler_setting LIMIT 0 , 30 ";
	$crawlerlink = sql_list($sql);
	$sum = count($crawlerlink);
	if($sum>0){
		$ns = round((int)date("i")/5)-1;	
	}else
		$ns = 0;
	if((int)date("i")>57)
		$ns = 11;
	//for($num=0;$num<$sum;$num++){
		$newscat = $crawlerlink[$ns]['catid'];
		$linknews = $crawlerlink[$ns]['url'];
		
		//$newscat = 464;
		//$linknews = "http://xalo.vn/news/tl/Giao-duc/40.htm";
		
	
		echo "<pre>";
//		print_r($crawlerlink);
//		echo "</pre>";
//		echo date("H")."/".$sum."=".$ns."<br />".$newscat."<br />".$linknews; 
