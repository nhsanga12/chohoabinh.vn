<?php global $config, $str,$pages;
	...
	if ($id['option'] == "main") {
		if($id['search']=='Từ khóa') $id['search'] = '';
		$sql = "SELECT art.id, art.category, art.lastdate, art.state_p, art.status, art.hot, art.bydate,
					detail.title, detail.ma, detail.gia, detail.khuyenmai,detail.thuonghieu, detail.quick, detail.contents
		...
		$sql.= "WHERE detail.title LIKE '%".addslashes($id['search'])."%' AND detail.language = 'vn' AND art.status <> '0' ";
		
		if($grpid !='')
		$sql.= "AND art.groupid = '".$grpid."' ";
		
		if($id['category'] !=''){
			$sql.= " AND ( art.category= '".$id['category'];
...
/*OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO*/	
	...
	if ($id['option'] == "add") {
		...
		if(isset($_POST['taothums'])){
			$oldfile = '../lib/articles/thums_'.$_SESSION['auth']['name'].'.'.$_SESSION['auth']['temp'];
			$oldfile_thum = '../lib/articles/thums_thums_'.$_SESSION['auth']['name'].'.'.$_SESSION['auth']['temp'];
			if(is_file($oldfile)){
				@unlink($oldfile);
				@unlink($oldfile_thum);
			}
				
			$thumpic = sys_uploads('../lib/articles/','add_picture','gif|jpg|png|GIF|JPG|PNG',2);
			if($thumpic!=''){
				$cut = explode(".",$thumpic); // cắt tên hình
				$tenmoi = "thums_".$_SESSION['auth']['name'].".".end($cut); // tạo tên mới
				$_SESSION['auth']['temp'] = end($cut);
				$rename = @rename("../lib/articles/".$thumpic,"../lib/articles/".$tenmoi);
			}else{
				$msg .= 'Vui lòng chọn hình để tạo thum';
			}
		...
		if (isset($_POST['submit'])) {
			...
			if ($msg == '') {
				...
				for ($i=0; $i < count($lgroups); $i++) {
					...
					if( $i>0 && $rsn['seotit']== $oldtk){
						$msg1 = ' .Tiêu đề vn và en giống nhau nên chỉ lưu 1 link vào sitemap !';
					}else{
						$config['default_language'] = $lgroups[$i]['key'];
						$newlink = CmsBreadcrumbLink('',$id['category'],$rsn['article'],'',sys_sign($rsn['title'])."_".$newid.".html");
				...
				for ($j=0;$j<$config['max_file'];$j++) {
					...
					if ($_POST[$title1] != ''){
						...
						$rsp['file']	=	sys_uploads($nhom,$file1,'gif|jpg|png|GIF|JPG|PNG|PDF|DOC|XLS|RAR|ZIP|PPT|swf|SWF|flv|FLV|txt|TXT',0,true);
						$rsp['article']	=	$newid;
	...
	if ($id['option'] == "edit") {
		...
		for ($i=0; $i < count($lgroups); $i++) {
			...
			if ($chk > 0) {
					...
					// sitemap general
					$oldtk = sys_sign($_POST['add_title_'.$lgroups[0]['key']]);
					if( $i>0 && $rsi['seotit']== $oldtk){
						$msg1 = ' .Tiêu đề vn và en giống nhau nên chỉ lưu 1 link vào sitemap !';
					}else{
						$config['default_language'] = $lgroups[$i]['key'];
						$newlink = CmsBreadcrumbLink('',$id['category'],$id['item'],'',sys_sign($rsi['title'])."_".$id['item'].".html");
						$rsi['sitemappoint'] = UpdateSitemap('../sitemap_detail.xml',$config['url'].$newlink,'weekly','0.8',$_POST['sitemappoint_'.$lgroups[$i]['key']]);
					}
					// END sitemap general					
					...
					$rsu['seotit']		= 	$_POST['add_seotit_'.$lgroups[$i]['key']];// Xử lý SEO ==================
					$rsu['seodes']		= 	$_POST['add_seodes_'.$lgroups[$i]['key']];// Xử lý SEO ==================
					$rsu['article']		=	$id['item'];
					$rsu['title']		= 	$_POST['add_title_'.$lgroups[$i]['key']];
					
					// sitemap general
					$oldtk = sys_sign($_POST['add_title_'.$lgroups[0]['key']]);
					if( $i>0 && $rsu['seotit']== $oldtk){
						$msg1 = ' .Tiêu đề vn và en giống nhau nên chỉ lưu 1 link vào sitemap !';
					}else{
						$config['default_language'] = $lgroups[$i]['key'];
						$newlink = CmsBreadcrumbLink('',$id['category'],$id['item'],'',sys_sign($rsu['title'])."_".$id['item'].".html");
						$rsu['sitemappoint'] = AddSitemap('../sitemap_detail.xml',$config['url'].$newlink,'weekly','0.8');
					}
					// END sitemap general
		...
		for ($j=0;$j<$config['max_file'];$j++) {
				...
				if ($_POST[$title1] != ''){
					...
					$rsp['title']	=	$_POST[$title1];
					$rsp['quick']	=	$_POST[$quick1];					 
					$rsp['file']	=	sys_uploads($nhom,$file1,'gif|jpg|png|GIF|JPG|PNG|PDF|DOC|XLS|RAR|ZIP|PPT|swf|SWF|flv|FLV|txt|TXT',0,$crop);
					$rsp['article']	=	$id['item'];
					$rsp['type']	=	$_POST['nhomfile'];
					$rsp['volum']	=	$_FILES[$file1]["size"];
					$rsp['thum']	=	sys_uploads($nhom,$thumnail1,'gif|jpg|png|GIF|JPG|PNG|PDF|DOC|XLS|RAR|ZIP|PPT|swf|SWF|flv|FLV|txt|TXT',1,$crop);
					if($rsp['thum']=='')
					$rsp['thum']	=	'thums_'.$rsp['file'];
					if($rsp['file']!='') $picid = sql_add('file',$rsp);
				}
		...
		$sql = "SELECT art.id, art.picture, art.lastdate, art.status, art.state_p, art.hot, art.opt, detail.language, detail.title, detail.ma,detail.loaitien, detail.gia,detail.khuyenmai,detail.thuonghieu, detail.quick,detail.seotit,detail.seodes,detail.contents,detail.sitemappoint FROM ".$config['db_prefix']."_news_articles art RIGHT JOIN ".$config['db_prefix']."_news_articles_detail detail ON art.id = detail.article ";
		$sql.= "WHERE art.id = '".$id['item']."' AND art.status <> '0' ";
...
/*OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO*/	
	...
	if ($id['option'] == 'delete' && $id['dupple'] == 0) {
		...
		if ($_POST['submit']) {
			...
			echo "<script>window.location='?gnc=com:".$id['com'].";target:".$id['target'].";option:main;limit_on_page:".$id['limi_on_page'].";page:".$id['page'].";search:".$id['search'].";groups:".$id['groups'].";category:".$id['category']."';</script>";
		}
		while (list($key,$value)=each($item)) {
