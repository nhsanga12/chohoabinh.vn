<?php
	if ($id['option'] == "main") {
		$sql = "SELECT grp.*, detail.title FROM ".$config['db_prefix']."_news_groups grp RIGHT JOIN ".$config['db_prefix']."_news_groups_detail detail ON grp.id = detail.groupid ";
		$sql.= "WHERE title LIKE '%".addslashes($id['search'])."%' AND detail.language = '".$config['default_language']."' ORDER BY grp.id ASC ";
		...
		if (isset($_POST['submit'])) {
			...
			if ($msg == '') {
				...
				for ($i=0; $i < count($lgroups); $i++) {
					...
					$rsn['name']		= 	$_POST['add_name_'.$lgroups[$i]['key']];
	...
	if ($id['option'] == "edit") {
		...
		for ($i=0; $i < count($lgroups); $i++) {
			...
			if ($chk > 0) {
					...
					$rsi['name']		= 	$_POST['add_name_'.$lgroups[$i]['key']];
					...
					$rsu['name']		= 	$_POST['add_name_'.$lgroups[$i]['key']];
		...
		$sql = "SELECT grp.id, detail.title, detail.name, grp.lastdate, detail.language, grp.oderid FROM ".$config['db_prefix']."_news_groups grp RIGHT JOIN ".$config['db_prefix']."_news_groups_detail detail ON grp.id = detail.groupid ";
