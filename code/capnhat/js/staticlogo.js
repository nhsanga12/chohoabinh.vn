...
var ddsitelogo={
	...
	offsets: {x:0, y:0},//canh le cua logo so voi goc duoi
	logoHTML: 'HTML Chua dc setup', //HTML for logo, which is auto wrapped in DIV w/ ID="mysitelogo"
	...
	init:function(){
		jQuery(document).ready(function($){
			...
			if (mainobj.cssfixedsupport){
				mainobj.coord[(/^[13]$/.test(mainobj.setting.orientation))? 'left' : 'left']=mainobj.offsets.x
				mainobj.coord[(/^[12]$/.test(mainobj.setting.orientation))? 'top' : 'top']=mainobj.offsets.y
			}
			mainobj.$control=$('<div id="mysitelogo" style="z-index:300;">'+mainobj.logoHTML+'</div>')
